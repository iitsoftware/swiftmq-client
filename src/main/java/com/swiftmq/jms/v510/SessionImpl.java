...
public class SessionImpl
        implements Session, RequestService, QueueSession, TopicSession, SwiftMQSession, SessionExtended {
    ...
    public void setCurrentTransaction(List current) {
        synchronized (transactedRequestList) {
            transactedRequestList.clear();
            if (current != null) {
                transactedRequestList.addAll(current);
            }
        }
    }
        ...
        try {
            ...
            reply =
                    (CreateSubscriberReply) requestRegistry.request(new CreateSubscriberRequest(dispatchId,
                            (TopicImpl) topic, ms, noLocal, true));
        } catch (Exception e) {
            throw ExceptionConverter.convert(e);
        }
    ...
    private synchronized void doDeliverMessage(AsyncMessageDeliveryRequest request) {
        ...
        if (consumer != null) {
            if (SMQPUtil.isBulk(request))
                consumer.addToCache(SMQPUtil.createRequests(request), request.isRequiresRestart());
            else
    ...
    private class SessionDeliveryQueue extends SingleProcessorQueue {
        ...
        private MessageConsumerImpl[] toArray() {
            if (ci == null || ci.length < consumerMap.size())
                ci = new MessageConsumerImpl[consumerMap.size()];
            int i = 0;
            for (Iterator iter = consumerMap.entrySet().iterator(); iter.hasNext(); ) {
                ci[i++] = (MessageConsumerImpl) ((Map.Entry) iter.next()).getValue();
            }
            return ci;
        }
