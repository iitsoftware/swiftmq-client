...
public class ConnectionImpl extends RequestServiceRegistry
    ...
    public static final int CLIENT_VERSION = 400;
    public static final int DISCONNECTED = -1;
    public static final int CONNECTED_STOPPED = 0;
    public static final int CONNECTED_STARTED = 1;
    static final KeepAliveRequest keepaliveRequest = new KeepAliveRequest();
    ...
    DumpableFactory dumpableFactory = new com.swiftmq.jms.smqp.SMQPFactory(new com.swiftmq.jms.smqp.v400.SMQPFactory());
    ...
    protected ConnectionImpl(String userName, String password, com.swiftmq.net.client.Connection connection)
        ...
        try {
            ...
            GetMetaDataReply mdReply =
                    (GetMetaDataReply) requestRegistry.request(new GetMetaDataRequest());

            if (mdReply.isOk()) {
                metaData = mdReply.getConnectionMetaData();
            } else {
        ...
        if (clientID != null) {
            ...
            try {
                SetClientIdReply reply =
                        (SetClientIdReply) requestRegistry.request(new SetClientIdRequest(clientID));
    ...
    protected void authenticate() throws Exception {
        try {
            GetAuthChallengeReply gaReply = (GetAuthChallengeReply) requestRegistry.request(new GetAuthChallengeRequest(userName));
            Serializable challenge = null;
            if (gaReply.isOk()) {
                challenge = gaReply.getChallenge();
                crFactory = (ChallengeResponseFactory) Class.forName(gaReply.getChallengeResponseFactoryClass()).newInstance();
            } else
                throw gaReply.getException();
            Serializable response = crFactory.createResponse(challenge, password);
            AuthResponseReply arReply = (AuthResponseReply) requestRegistry.request(new AuthResponseRequest(response));
            if (!arReply.isOk())
    ...
    public void deleteTempQueue(String queueName) throws JMSException {
        Reply reply = null;
        try {
            reply = requestRegistry.request(new DeleteTmpQueueRequest(queueName));
        } catch (Exception e) {
            throw ExceptionConverter.convert(e);
        }
    ...
    public ConnectionConsumer createConnectionConsumer(Queue queue,
        ...
        try {
            reply = (CreateSessionReply) requestRegistry.request(new CreateSessionRequest(false,
    ...
    public void setClientID(String s) throws JMSException {
        ...
        try {
            reply = (SetClientIdReply) requestRegistry.request(new SetClientIdRequest(s));
        } catch (Exception e) {
    ...
    private void writeObject(Dumpable obj) throws Exception {
        Dumpalizer.dump(outStream, (Dumpable) obj);
        outStream.flush();
    }
    ...
    public void onException(IOException exception) {
        ...
        if (exceptionListener != null) {
            exceptionListener.onException(new JMSException(exception.getMessage()));
    ...
    public void dataAvailable(LengthCaptureDataInput in) {
        try {
            ...
            if (exceptionListener != null) {
                exceptionListener.onException(new JMSException(e.getMessage()));
                exceptionListener = null;
            }
        }
    }
