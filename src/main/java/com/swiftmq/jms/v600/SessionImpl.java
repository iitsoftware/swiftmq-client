...
public class SessionImpl
    ...
    private void _close() {
        ...
        try {
            requestRegistry.request(new CloseSessionRequest(this, 0, dispatchId));
        } catch (Exception e) {
        }
    ...
    public void run() {
        ...
        while ((lastMessage = nextMessageChunk()) != null) {
            try {
                ...
                if (MessageTracker.enabled) {
                    MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, " + lastMessage.getConnectionId() + " / " + myConnection.getConnectionId());
                }
                if (lastMessage.getConnectionId() != myConnection.getConnectionId()) {
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, invalid connectionId (" + lastMessage.getConnectionId() + " vs " + myConnection.getConnectionId() + ")");
                    }
                ...
                boolean duplicate = connectionConsumer.isDuplicate(lastMessage.getMessage());
                if (autoAssign && duplicate) {
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, duplicate, continue with next message!");
                    }
                ...
                if (xaMode && duplicate) {
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, duplicate!");
                    }
                    cancelled = assignLastMessage(true);
                } else {
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, onMessage...");
                    }
                    withinOnMessage = true;
                    messageListener.onMessage(lastMessage.getMessage());
                    withinOnMessage = false;
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, onMessage ok");
                    }
                }
                // eventually ack message when auto-ack
                if (!cancelled && !transacted && acknowledgeMode != Session.CLIENT_ACKNOWLEDGE) {
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, ack ...");
                    }
                    cancelled = acknowledgeMessage(lastMessage.getMessageIndex());
                    if (MessageTracker.enabled) {
                        MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, ack done, cancelled=" + cancelled);
                    }
                    if (cancelled) {
                        setRunning(false);
                        return;
                ...
                withinOnMessage = false;
                if (MessageTracker.enabled) {
                    MessageTracker.getInstance().track(lastMessage.getMessage(), new String[]{myConnection.toString(), toString()}, "run, exception=" + e);
                }
                setRunning(false);
                return;
            }
    ...
    private class SessionDeliveryQueue extends SingleProcessorQueue {
        ...
        RingBuffer consumerCopy = new RingBuffer(32);
        ...
        private void copyConsumers() {
            consumerCopy.clear();
            synchronized (SessionImpl.this) {
                for (Iterator iter = consumerMap.entrySet().iterator(); iter.hasNext(); ) {
                    consumerCopy.add(((Map.Entry) iter.next()).getValue());
                }
            }
        }
        ...
        protected void process(Object[] bulk, int n) {
            ...
            copyConsumers();
            while (consumerCopy.getSize() > 0 && valid()) {
                MessageConsumerImpl c = (MessageConsumerImpl) consumerCopy.remove();
