...
public class EndpointImpl extends RequestServiceRegistry
    ...
    public boolean isSubscriptionFilterEnabled() {
        return subscriptionFilterEnabled;
    }
    ...
    public void contextShown(String[] context, boolean includeNextLevel) {
        if (subscriptionFilterEnabled) {
            String s = SwiftUtilities.concat(context, "/") + "?" + includeNextLevel;
            SubscriptionCounter c = (SubscriptionCounter) subscriptions.get(s);
            if (c == null) {
                c = new SubscriptionCounter();
                subscriptions.put(s, c);
                requestRegistry.request(new SetSubscriptionFilterRequest(context, includeNextLevel));
            }
            c.cnt++;
        }
    }
        ...
        if (subscriptionFilterEnabled) {
            ...
            if (c != null) {
                ...
                if (c.cnt == 0) {
                    ...
                    requestRegistry.request(new RemoveSubscriptionFilterRequest(context, includeNextLevel));
                }
            }
        }
    }
